...
A release binary carries the same three folders inside it. The first time you run it on
an empty data directory it writes them to `apps/` there, so the launcher has something to
show and you have something to edit; `privatium new --examples` writes them again if you
want a fresh copy (`spec/cli.md §2`, `§4`).

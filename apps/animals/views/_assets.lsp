<?-- Project: Privatium™ | apps/animals/views/_assets.lsp
     Summary: The three files this app adds to the framework's own. Included at the top of
              every page template rather than injected into <head>, because `defer` makes
              position irrelevant to layout and a Tier 1 app has no reason to own the
              document shell.

              HTMX is the framework's, already loaded — do not vendor a second copy. Alpine
              is this app's, and must be the CSP build (see static/VENDOR.md).
              ...
              reset form are simply on the page. A link, not an inline <style>, because the
              default CSP has no style-src (spec/protocol.md §9.3).
